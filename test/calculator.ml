...
type intlist = { ints: int32 list }

let rec pp_intlist out (self : intlist) =
  Format.fprintf out "{@[";
  Format.fprintf out "ints=";
  pp_list (fun out x -> Format.fprintf out "%ld" x) out self.ints;
  Format.fprintf out "@]}"
...
let rec read_intlist (module IP : PROTOCOL_READ) : intlist =
  ...
  let ints =
    ...
    | None ->
      raise
        (Runtime_error (UE_invalid_protocol, {|field (1: "ints") is required|}))
    | Some x -> x
  in
  { ints }
      ...
      try
        ...
        | "add", MSG_CALL ->
          ...
          while !continue do
            match IP.read_field_begin () with
            | exception Thrifty.Types.Read_stop_field -> continue := false
            | "a", T_I32, _ | _, T_I32, 1 -> a := Some (IP.read_i32 ())
            | "a", _, _ | _, _, 1 ->
              raise
                (Runtime_error
                   (UE_invalid_protocol, {|invalid type for field (1: "a")|}))
            | "b", T_I32, _ | _, T_I32, 2 -> b := Some (IP.read_i32 ())
            | "b", _, _ | _, _, 2 ->
              raise
                (Runtime_error
          ...
          let a =
            ...
            | None ->
              raise
                (Runtime_error
                   (UE_invalid_protocol, {|field (1: "a") is required|}))
            | Some x -> x
          in
          let b =
            match !b with
            | None ->
              raise
                (Runtime_error
                   (UE_invalid_protocol, {|field (2: "b") is required|}))
            | Some x -> x
          in
          ...
          in
          (* call the user code *)
          (try self#add ~a ~b () ~reply with e -> reply (Error e))
        | "div", MSG_CALL ->
          (* read arguments *)
          let _name = IP.read_struct_begin () in
          let continue = ref true in
          ...
          in
          (* call the user code *)
          (try self#div ~a ~b () ~reply with e -> reply (Error e))
        | "add_all", MSG_CALL ->
          (* read arguments *)
          let _name = IP.read_struct_begin () in
          let continue = ref true in
          ...
          let l =
            ...
            | None ->
              raise
                (Runtime_error
                   (UE_invalid_protocol, {|field (1: "l") is required|}))
            | Some x -> x
          in
          let reply (x : _ result) : unit =
            match x with
          ...
          in
          (* call the user code *)
          (try self#add_all ~l () ~reply with e -> reply (Error e))
        | "ping", MSG_ONEWAY ->
          (* read arguments *)
          let _name = IP.read_struct_begin () in
          let continue = ref true in
        ...
        | "get_pings", MSG_CALL ->
          ...
          in
          (* call the user code *)
          (try self#get_pings () ~reply with e -> reply (Error e))
        | _n, _ ->
