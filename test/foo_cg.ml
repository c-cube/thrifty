...
type foo = { x: int32 option; y: string option; z: bool }

let rec pp_foo out (self : foo) =
  Format.fprintf out "{@[";
  Option.iter
    (fun x ->
      Format.fprintf out {|x=|};
      Format.fprintf out "%ld" x)
    self.x;
  Format.fprintf out ";@ ";
  Option.iter
    (fun x ->
      Format.fprintf out {|y=|};
      Format.fprintf out "%S" x)
    self.y;
  Format.fprintf out ";@ ";
  Format.fprintf out "z=";
  Format.fprintf out "%B" self.z;
  Format.fprintf out "@]}"
...
let rec read_foo (module IP : PROTOCOL_READ) : foo =
  ...
  while !continue do
    ...
    | "x", T_I32, _ | _, T_I32, 2 -> x := Some (IP.read_i32 ())
    | "x", _, _ | _, _, 2 ->
      raise
        (Runtime_error (UE_invalid_protocol, {|invalid type for field (2: "x")|}))
    | "y", (T_STRING | T_BINARY), _ | _, (T_STRING | T_BINARY), 1 ->
      y := Some (IP.read_string ())
    | "y", _, _ | _, _, 1 ->
      raise
        (Runtime_error (UE_invalid_protocol, {|invalid type for field (1: "y")|}))
    | "z", T_BOOL, _ | _, T_BOOL, 5 -> z := Some (IP.read_bool ())
    | "z", _, _ | _, _, 5 ->
      raise
        (Runtime_error (UE_invalid_protocol, {|invalid type for field (5: "z")|}))
    | _ -> ()
    (* unknown field *)
  done;
  IP.read_struct_end ();
  ...
  let z =
    ...
    | None ->
      raise
        (Runtime_error (UE_invalid_protocol, {|field (5: "z") is required|}))
    | Some x -> x
  in
  { x; y; z }

type loc = { long: float; lat: float }

let rec pp_loc out (self : loc) =
  Format.fprintf out "{@[";
  Format.fprintf out "long=";
  Format.fprintf out "%f" self.long;
  Format.fprintf out ";@ ";
  Format.fprintf out "lat=";
  Format.fprintf out "%f" self.lat;
  Format.fprintf out "@]}"
...
let rec read_loc (module IP : PROTOCOL_READ) : loc =
  ...
  while !continue do
    match IP.read_field_begin () with
    | exception Thrifty.Types.Read_stop_field -> continue := false
    | "long", T_DOUBLE, _ | _, T_DOUBLE, 1 -> long := Some (IP.read_double ())
    | "long", _, _ | _, _, 1 ->
      raise
        (Runtime_error
           (UE_invalid_protocol, {|invalid type for field (1: "long")|}))
    | "lat", T_DOUBLE, _ | _, T_DOUBLE, 2 -> lat := Some (IP.read_double ())
    | "lat", _, _ | _, _, 2 ->
      raise
        (Runtime_error
  ...
  let long =
    ...
    | None ->
      raise
        (Runtime_error (UE_invalid_protocol, {|field (1: "long") is required|}))
    | Some x -> x
  in
  let lat =
    match !lat with
    | None ->
      raise
        (Runtime_error (UE_invalid_protocol, {|field (2: "lat") is required|}))
    | Some x -> x
  in
  { long; lat }

exception Ohno
exception Ohno2 of { really_bad: bool option }
exception Ohno3 of { why: string option; where: loc option }
...
let fooK_of_int = function
  ...
  | n ->
    raise
      (Runtime_error
         ( UE_invalid_protocol,
           Printf.sprintf "unknown enum member %d for `fooK`" n ))
...
(** Deserialize a "fooK" *)
let read_fooK (module IP : PROTOCOL_READ) : fooK = IP.read_i16 () |> fooK_of_int
...
let rec pp_bar out (self : bar) =
  ...
  Option.iter
    (fun x ->
      Format.fprintf out {|foos=|};
      pp_list (pp_list pp_foo) out x)
    self.foos;
  Format.fprintf out ";@ ";
  Option.iter
    (fun x ->
      Format.fprintf out {|kind=|};
      pp_fooK out x)
    self.kind;
  Format.fprintf out ";@ ";
  Format.fprintf out "fooM=";
  pp_list (pp_pair pp_fooK (pp_list pp_foo)) out self.fooM;
  Format.fprintf out "@]}"
...
let rec read_bar (module IP : PROTOCOL_READ) : bar =
  ...
  while !continue do
    ...
    | "fooM", T_MAP, _ | _, T_MAP, 3 ->
      fooM :=
        Some
          (let tyk, tyv, len = IP.read_map_begin () in
           assert (
             len = 0
             ||
             match tyk, tyv with
             | T_STRUCT, T_LIST -> true
             | _ -> false);
           List.init len (fun _i ->
  ...
  let fooM =
    ...
    | None ->
      raise
        (Runtime_error (UE_invalid_protocol, {|field (3: "fooM") is required|}))
    | Some x -> x
  in
  { foos; kind; fooM }

type fooOrBarOrBool = Foo of foo | Bar of bar | B of bool
...
let rec pp_fooOrBarOrBool out (self : fooOrBarOrBool) =
  ...
  | B self ->
    Format.fprintf out "B (%a)" (fun out x -> Format.fprintf out "%B" x) self

(** Serialize *)
let rec write_fooOrBarOrBool (module OP : PROTOCOL_WRITE)
    (self : fooOrBarOrBool) : unit =
...
let rec read_fooOrBarOrBool (module IP : PROTOCOL_READ) : fooOrBarOrBool =
  ...
  while !continue do
    match IP.read_field_begin () with
    | exception Thrifty.Types.Read_stop_field -> continue := false
    | "foo", T_STRUCT, _ | _, T_STRUCT, 1 -> foo := Some (read_foo (module IP))
    | "foo", _, _ | _, _, 1 ->
      raise
        (Runtime_error
           (UE_invalid_protocol, {|invalid type for field (1: "foo")|}))
    | "bar", T_STRUCT, _ | _, T_STRUCT, 2 -> bar := Some (read_bar (module IP))
    | "bar", _, _ | _, _, 2 ->
      raise
        (Runtime_error
           (UE_invalid_protocol, {|invalid type for field (2: "bar")|}))
    | "b", T_BOOL, _ | _, T_BOOL, 3 -> b := Some (IP.read_bool ())
    | "b", _, _ | _, _, 3 ->
      raise
        (Runtime_error (UE_invalid_protocol, {|invalid type for field (3: "b")|}))
    | _ -> ()
    (* unknown field *)
  done;
  IP.read_struct_end ();
...
class virtual server_giveKind =
  object (self)
    ...
    method process (ip : protocol_read)
        ~(reply : (protocol_write -> unit) -> unit) : unit =
      let (module IP) = ip in
      ...
      try
        ...
        | "get_kind", MSG_CALL ->
          ...
          in
          (* call the user code *)
          (try self#get_kind ?foo () ~reply with e -> reply (Error e))
        | "send_bar", MSG_CALL ->
          (* read arguments *)
          let _name = IP.read_struct_begin () in
          let continue = ref true in
          ...
          in
          (* call the user code *)
          (try self#send_bar ?bar () ~reply with e -> reply (Error e))
        | "send_whatev", MSG_ONEWAY ->
          ...
          while !continue do
            ...
            | "how_many", _, _ | _, _, 1 ->
              raise
                (Runtime_error
                   ( UE_invalid_protocol,
                     {|invalid type for field (1: "how_many")|} ))
          ...
          let how_many =
            ...
            | None ->
              raise
                (Runtime_error
                   (UE_invalid_protocol, {|field (1: "how_many") is required|}))
            | Some x -> x
          in
          let k = !k in
...
module Client_giveKind : sig
    ...
    let read_reply (module IP : PROTOCOL_READ) : fooK =
      ...
      | MSG_REPLY ->
        ...
        let _, ty, f_id =
          ...
          with Read_stop_field ->
            raise (Runtime_error (UE_missing_result, {|expected a field|}))
        in
        (match f_id with
        | 0 ->
          let res = read_fooK (module IP) in
          IP.read_struct_end ();
          res
        | _id ->
          raise
      ...
      | MSG_EXCEPTION ->
        ...
        while !continue do
          match IP.read_field_begin () with
          | exception Thrifty.Types.Read_stop_field -> continue := false
          | "type", T_I32, _ | _, T_I32, 0 -> ty := Some (IP.read_i32 ())
          | "type", _, _ | _, _, 0 ->
            raise
              (Runtime_error
        ...
        let ue =
          match !ty with
          | None -> UE_unknown
          | Some ty -> unexpected_exception_of_int ty
        in
        raise (Runtime_error (ue, msg))
      | _ ->
        raise
          (Runtime_error (UE_invalid_protocol, {|expected reply or exception|}))
    in
    (), read_reply
   ...
   fun ~seq_num (module OP : PROTOCOL_WRITE) ->
    ...
    let read_reply (module IP : PROTOCOL_READ) : unit =
      ...
      | MSG_REPLY ->
        ...
        (try
           let _, ty, f_id =
             try IP.read_field_begin () with Read_stop_field -> raise Exit
           in
           match f_id with
           | 0 ->
             raise
               (Runtime_error
                  (UE_invalid_protocol, {|void method should have no return|}))
           | 2 -> raise Ohno
           | 3 ->
             let continue = ref true in
  ...
  let send_whatev ~(how_many : int32) ?(k : fooK option) :
      client_outgoing_oneway =
   fun ~seq_num (module OP : PROTOCOL_WRITE) ->
...
class virtual server_calculator =
  object (self)
    inherit service_any
      ...
      try
        ...
        | "add", MSG_CALL ->
          ...
          while !continue do
            match IP.read_field_begin () with
            | exception Thrifty.Types.Read_stop_field -> continue := false
            | "x", T_I32, _ | _, T_I32, 1 -> x := Some (IP.read_i32 ())
            | "x", _, _ | _, _, 1 ->
              raise
                (Runtime_error
                   (UE_invalid_protocol, {|invalid type for field (1: "x")|}))
            | "y", T_I32, _ | _, T_I32, 2 -> y := Some (IP.read_i32 ())
            | "y", _, _ | _, _, 2 ->
              raise
                (Runtime_error
          ...
          let x =
            ...
            | None ->
              raise
                (Runtime_error
                   (UE_invalid_protocol, {|field (1: "x") is required|}))
            | Some x -> x
          in
          let y =
            match !y with
            | None ->
              raise
                (Runtime_error
                   (UE_invalid_protocol, {|field (2: "y") is required|}))
            | Some x -> x
          in
          let reply (x : _ result) : unit =
            match x with
          ...
          in
          (* call the user code *)
          (try self#add ~x ~y () ~reply with e -> reply (Error e))
        | "mult", MSG_CALL ->
          (* read arguments *)
          let _name = IP.read_struct_begin () in
          let continue = ref true in
          ...
          in
          (* call the user code *)
          (try self#mult ~x ~y () ~reply with e -> reply (Error e))
        | _n, _ ->
          raise (Runtime_error (UE_invalid_message_type, {|invalid message|}))
      with Runtime_error (ue, msg) ->
        (* catch runtime errors and reify them *)
